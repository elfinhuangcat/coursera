...
class TestGraphFunctions(unittest.TestCase):
    """
    Class name: TestGraphFunctions
    Usage: Test the graph functions defined above. 
    Tests:
    """
    # There is no setUp since we will use the global constants.
    
    def test_make_complete_graph(self):
        """
        Function name: test_make_complete_graph
        Goal: Test the make_complete_graph defined above.
        """
        self.assertDictEqual(make_complete_graph(0), dict(),
                             "Empty graph test. Output should be set([])")
        self.assertDictEqual(make_complete_graph(1),
                             {0:set()},
                             "Only 1 node. No edge because cyclic edge" +
                             "is not permitted.")
        self.assertDictEqual(make_complete_graph(2),
                             {0:set([1]), 1:set([0])},
                             "2 nodes test.")
    
    def test_compute_in_degrees(self):
        """
        Function name: test_compute_in_degrees
        Goal: Test the compute_in_degrees defined above.
        """
        self.assertDictEqual(compute_in_degrees(dict()), dict(),
                             "Empty graph test.")
        self.assertDictEqual(compute_in_degrees({0:set()}),
                             {0:0},
                             "With only one node. No edges. 0 in-degree")
        self.assertDictEqual(compute_in_degrees(EX_GRAPH0),
                             {0:0, 1:1, 2:1},
                             "Test EX_GRAPH0")
        self.assertDictEqual(compute_in_degrees(EX_GRAPH1),
                             {0:1, 1:2, 2:2, 3:1, 4:1, 5:1, 6:1},
                             "Test EX_GRAPH1")
        
    def test_in_degree_distribution(self):
        """
        Function name: test_in_degree_distribution
        Goal: Test the function `in_degree_distribution` defined above.
        """
        self.assertDictEqual(dict(), dict(), "Empty graph test.")
        self.assertDictEqual(in_degree_distribution({0:set()}), {0:1},
                             "A graph with only 1 node and no edges." + 
                             "\nShould return an empty distribution dict.")
        self.assertDictEqual(in_degree_distribution(EX_GRAPH0),
                             {0:1,1:2},
                             "Test EX_GRAPH0. Two nodes with in-degree 1.")
